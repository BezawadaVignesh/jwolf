...
public class Interptr {
	...
	WolfObj runFunction(WolfObj obj, ArrayList<AST> args) {
		...
		if(obj.getType() == TokenType.FUNC){
			...
			if(func.getKey().size() >= args.size()) {
				try {
						return run_block(func.getValue(), Map.entry((func.getKey()).clone(), args)); 
	...
	WolfObj i_AssignOp(parser.AssignOp t) {
		...
		if(t.get_name().size() == ((ArrayList<WolfObj>)obj.getValue()).size()) {
			...
			for(int i=0;i<t.get_name().size();i++) {
				...
				}else {throw new ParserError("Dev Error Assignment Names can only be CToken or UnPackOp got : "+(t.get_name().get(i)).type);}
			}
		}
	...
	WolfObj run_block(ArrayList<AST> stms, Entry args) {
		this.varLists.addFirst(new HashMap<String, WolfObj>());
		ArrayList<AST> names = (ArrayList<AST>)args.getKey(), values = (ArrayList<AST>)args.getValue();
		ArrayList<AST> finalNames = new ArrayList<AST>(), finalValues = new ArrayList<AST>();
		int i=0;
		// adds normal vars to final list
		for(i=0;i<values.size();i++) {
			if(values.get(i).type == OpType.ASSIGNOP) 
				break;
		...
		for(;i< values.size();i++) {
			if(values.get(i).type != OpType.ASSIGNOP) 
				throw new parser.ParserError("SyntaxError: non-default argument follows default argument");
			if(!(names.remove(((parser.AssignOp)values.get(i)).get_name().get(0)))) //
				throw new parser.ParserError("TypeError: func() got an unexpected keyword argument "+((parser.AssignOp)values.get(i)).get_name().get(0));
			i_AssignOp((parser.AssignOp)values.get(i));
			
		}
		
		for(i=0;i<names.size();i++) {
			if(names.get(i).type == OpType.ASSIGNOP) 
				break;
			finalNames.add(names.get(i));
		}
		if(finalNames.size() > finalValues.size() || names.size() < finalValues.size()) 
			throw new parser.ParserError("TypeError: Args mismatch func() takes "+finalNames.size()+" but got "+values.size());
		for(;i<finalValues.size();i++) {
			if(names.get(i).type != OpType.ASSIGNOP) 
				throw new parser.ParserError("SyntaxError: non-default argument follows default argument");
			finalNames.add(((parser.AssignOp)names.get(i)).get_name().get(0));
		}
		for(int j = i; j<names.size();j++) {
			if(names.get(i).type != OpType.ASSIGNOP) 
				throw new parser.ParserError("SyntaxError: non-default argument follows default argument");
			i_AssignOp((parser.AssignOp) names.get(j));
			
		}
		if(finalNames.size() < finalValues.size()) throw new parser.ParserError("Do Something Dev");
		i_AssignOp(new parser.AssignOp(finalNames, new CToken( TokenType.PACKED, finalValues )));
